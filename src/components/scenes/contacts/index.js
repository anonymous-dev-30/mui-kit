...
const Contacts = () => {
      ...
      renderCell: ({ row: { access } }) => {
        return (
          ...
          >
            {access === 'admin' && (
              <AdminPanelSettingsOutlined
                sx={{ color: colors.grey[100], mr: '5px' }}
              />
            )}
            {access === 'manager' && (
              <SecurityOutlined sx={{ color: colors.grey[100], mr: '5px' }} />
            )}
            {access === 'user' && (
              <LockOpenOutlined sx={{ color: colors.grey[100], mr: '5px' }} />
            )}
            <Typography
  ...
  return (
    <Box m="20px" height="75vh">
      <PageHeader title={'CONTACTS'} subTitle={'Managing team CONTACTS'} />
